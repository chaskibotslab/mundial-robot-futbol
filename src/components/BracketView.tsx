...
export default function BracketView({ matches, countries }: { matches: Match[]; countries: Record<string, Country> }) {
        ...
        {stages.map(stage => {
          ...
          return (
            <div key={stage} className="flex flex-col gap-3 min-w-[220px]">
              ...
              {list.map(m => <BracketMatch key={m.id} match={m} countries={countries} />)}
            </div>
          );
        })}
      </div>
...
function BracketMatch({ match, countries }: { match: Match; countries: Record<string, Country> }) {
  ...
  return (
    <div className={`card p-3 text-sm ${isLive ? "border-red-500" : ""}`}>
      <Side country={home} score={match.home_score} bold={isDone} />
      <Side country={away} score={match.away_score} bold={isDone} />
      {isLive && <div className="text-xs text-red-400 mt-1 animate-pulse">EN VIVO</div>}
    </div>
  );
}

function Side({ country, score, bold }: { country: Country | null; score: number; bold: boolean }) {
  return (
    <div className="flex justify-between items-center gap-2 mt-1">
      <span className="flex items-center gap-2 truncate">
        {country?.flag_url && <img src={country.flag_url} alt="" className="w-5 h-3 object-cover rounded-sm" />}
